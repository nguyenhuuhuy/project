...
// lưu tài khoản vào local và cho phép đăng nhập

let registration = document.getElementById("registration");

// chạy vào local xét điều kiện ko có key
let listProduct = JSON.parse(localStorage.getItem("list"));
...
registration.addEventListener("click", function () {
  ...
  if (listProduct == null) {
    ...
    for (let i = 0; i < listProduct.length; i++) {
      if (listProduct[i].email == email.value) {
        flag = false;
        break;
      } else {
        flag = true;
    ...
    if (flag == true) {
      ...
      error_Email.style.display = (error_Email.style.display == "block") ?"none":"block"
    }
  }
});
